...
Perform classification with cross validation.
    ...
    ss = sklearn.cross_validation.ShuffleSplit(len(dataset.data),
                                               n_iter=cmd_options.n_iterations, test_size=cmd_options.test_size,
                                               random_state=1234)
    ...
    for train_index, test_index in ss:
        ...
        if (cmd_options.classifier == "linear_model"):
            text_clf = Pipeline([('vect', CountVectorizer(ngram_range=(1, 5), max_df=1.0)),
                                 ...
                                 ('clf', SGDClassifier(alpha=0.000001, penalty="l2")), ])
...
It performs experiences using SVM, Naive Bayes and Linear Model being dataset agnostic.
    ...
    cmd_parser.add_option("-D", "--dataset", type="string", action="store", dest="directory",
